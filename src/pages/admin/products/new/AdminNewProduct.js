...
const AdminNewProduct = () => {
  ...
  const handleSubmit = () => {
    ...
    if (!product.price) return setError("Enter a valid product price");

    if (!product.weight) return setError("Enter a valid product weight");
    ...
    if (
      !product.image ||
      !product.image.match(/(https?:\/\/)?.*(\.png|\.jpg|\.jpeg)/)
    )
