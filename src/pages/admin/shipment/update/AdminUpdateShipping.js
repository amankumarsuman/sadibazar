    ...
    <div className={styles["wrapper"]}>
      ...
      {(state === INPUT || state === SHIPPING_VIEW) && (
        <div className={"warning-box"}>
          <span className={"warning"}>Warning:</span> The shipping status cannot
          be rolled back to a previous state after it has been modified.
        </div>
      )}
      {state === INPUT && (
        <div className={styles["wrapper2"]}>
          ...
          />
          <div onClick={handleFind} className={"btn1"}>
            Find
          </div>
        </div>
      )}
      {state === SHIPPING_VIEW && (
        <div className={styles["wrapper2"]}>
            ...
            >
              {options.map((option, i) => (
                <option
                  ...
                  key={i}
                  disabled={
                    options.indexOf(option) < options.indexOf(shipping.status)
                  }
                >
                  {option}
                </option>
          ...
          <div onClick={handleConfirm} className={"btn1"}>
            Confirm Update
          </div>
        </div>
      )}
      {state === SUCCESS && (
        <div className={styles["response-wrapper"]}>
          ...
          <div className={"btn2"} onClick={handleReset}>
            Update More
          </div>
        </div>
      )}
      {state === FAILURE && (
        <div className={styles["response-wrapper"]}>
          ...
          <p>
            Failed to update order #{id}. {failureMessage}
          </p>
          <div className={"btn2"} onClick={handleReset}>
            Retry
          </div>
