    ...
    <div className={styles["wrapper"]}>
      ...
      {(state === INPUT || state === ORDER_VIEW) && (
        <div className={"warning-box"}>
          <span className={"warning"}>Warning:</span> The order status cannot be
          rolled back to a previous state after it has been modified.
        </div>
      )}
      {state === INPUT && (
            ...
            >
              {options.map((option, i) => (
                <option
                  ...
                  key={i}
                  disabled={
                    options.indexOf(option) < options.indexOf(order.status)
                  }
                >
                  {option}
                </option>
